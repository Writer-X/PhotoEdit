...
public class TextObject extends ImageObject
{
	...
	public TextObject(Context context, String text, int x, int y,Bitmap rotateBm, Bitmap deleteBm)
	{
		super(text);
	...
	public void regenerateBitmap()
	{
		...
		if (srcBm != null)
			srcBm.recycle();
		srcBm = Bitmap.createBitmap(textWidth, textSize * (lines.length) + 8,Bitmap.Config.ARGB_8888);
	...
	public Typeface getTypefaceObj()
	{
		...
		if (typeface != null)
		{
			if (OperateConstants.CAOSHU.equals(typeface) || OperateConstants.GUIFANZI.equals(typeface))
			{
				tmptf = Typeface.createFromAsset(context.getAssets(), "fonts/" + typeface + ".ttf");
			}
		}
		if (bold && !italic)
